...
export default async function traverseChains(amount, fromChain, endpointId) {
    ...
    // gets: gas estimate
    const payableAmount = await endpointContract.methods.estimateFees(endpointId, LZFMULTI_ADDRESS[fromChain], payload, false, parameters).call()
    if (!payableAmount) {
    ...
    const value = await tokenContract
        .methods
        .traverseChains(
            endpointId,
            amount
